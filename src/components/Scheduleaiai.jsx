...
export function Scheduleaiai({ entrevistas }) {
	...
	return (
		<>
			<hr className='border-[#484848]' />
			<div className='mx-[2rem] my-[4rem] flex '>
				<h3 className='text-xl font-semibold lg:text-left  text-center '>{translations[lang].programação}</h3>
				
				<div className='text-[#eaebde]  lg:flex  lg:justify-center '>
									...
									<div>
										...
										<ul className='flex flex-col gap-4 '>
											{episodes.map(ep => {
												...
												return (
													<li key={ep._id} onClick={() => toggle(ep._id)} className='relative border-[.5px] border-[rgb(102,101,102)]  rounded-[1.5rem] p-3  cursor-pointer'>
														<div className='flex flex-col lg:grid lg:grid-cols-8  gap-6  '>
															<div className='col-span-4 flex gap-3   '>
																<div className=''>
																	<span className=' text-sm opacity-80  '>{lang === 'pt' ? ep.horario?.inicio : horaParaEnFormat(ep.horario?.inicio)}</span>
																</div>

																<div className='flex-grow text-[1.1rem] lg:text-[1.2rem] text-[#eaebde] leading-[1.4] font-semibold'>{lang === 'pt' ? ep.titulo : ep.tituloEN}</div>
															</div>
															<div className='col-span-4  flex justify-between'>
																	...
																	<div className='flex flex-col gap-2  lg:mt-0'>
																		<div>
																			{(lang === 'pt' ? ep.clusters2 : ep.clusters2_EN) && (
																				<div className='inline-block  px-3 py-1 text-[0.8rem] text-[#cccccb] rounded-full border border-[#4c4c4b] '>
																					{lang === 'pt' ? ep.clusters2 : ep.clusters2_EN}
																				</div>
																			)}
																		</div>
																		<div className='flex gap-2 flex-wrap'>
																			{Array.isArray(lang === 'pt' ? ep.clusters : ep.clustersEN) &&
																				(lang === 'pt' ? ep.clusters : ep.clustersEN).map((cluster, index) => (
																					<div key={index} className='inline-block  px-3 py-1 text-[0.8rem] text-[#cccccb] rounded-full border border-[#4c4c4b] '>
																						{cluster}
																					</div>
																				))}
																		</div>
																...
																<div className=' items-start lg:flex hidden'>
																	<motion.div initial={false} animate={{ rotate: isOpen ? 90 : 180 }} transition={{ duration: 0.3 }}>
																		<svg width='16' height='16' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg' className='text-[#eaebde]'>
																			<path
																				d='M8.12 4.71L13.41 10L8.12 15.29C7.73 15.68 7.73 16.31 8.12 16.7C8.51 17.09 9.14 17.09 9.53 16.7L15.53 10.7C15.92 10.31 15.92 9.68 15.53 9.29L9.53 3.29C9.14 2.9 8.51 2.9 8.12 3.29C7.73 3.68 7.73 4.31 8.12 4.71Z'
														...
														<AnimatePresence initial={false}>
															{isOpen && (
																<motion.div
																	...
																	className='overflow-hidden mt-6 lg:mt-4 text-sm text-gray-300 lg:px-0 lg:pb-0 '
																>
